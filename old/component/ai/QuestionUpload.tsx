...
import { createQuestion, sendInteralEmail, updateQuestion } from '@/url/route/verification';
...
import { QuizEditModel2 } from '@/template/model/QuizEditModel2';
...
const QuestionUpload: React.FC = () => {
    ...
    const HandleQuiz = async (values: any, number: number) => {
        ...
        const options = values.options.map((a: any, d:number) => {
            return {
        ...
        await updateQuestion(values.id, quiz);
    ...
    const submitQuestionByAi = async () => {
        try {
            ...
            toast('Quiz successfully updated', 'success');
        } catch (error: any) {
            toast(error.response?.data?.message || 'Failed to update Quiz', 'error');
        } finally {
            setLoading(false);
        }
    };
            ...
            <Flex flex={1} direction="column">
                ...
                >
                    ...
                    <Center
                        ...
                        justifyContent={"space-between"}>
                        <QuizEditModel2 number={numberSelection} />
                        <HStack
                            mt={3}
                            p={3}
                            w="auto"
