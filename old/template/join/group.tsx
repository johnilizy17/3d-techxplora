...
import VerifyGroup from "@/pages/dashboard/verify_group";
...
const GroupJoin = () => {
    ...
    const initiateSignUp = async (values: any, { setSubmitting }: any) => {
        try {
            setSubmitting(true);
            const { data } = await verifyQroupCode(values.code)
            dispatch(setTemporaryStorage(data) as any)
            router.push(`/dashboard/verify_group`)
            toast("code successfully verfied", "success")
            // });
            setSubmitting(false);
        } catch (error: any) {
            console.log(error)
            toast(error.response.data.message || error?.message ||  'Group not found failed', 'error');
            setSubmitting(false);
        }
    };
