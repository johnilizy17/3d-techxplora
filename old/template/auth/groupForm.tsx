...
const GroupForm = () => {
    ...
    const initiateSignUp = async (values: any, { setSubmitting }: any) => {
        try {
            setSubmitting(true);
            await dispatch(VerifyAdminCode(values) as any).unwrap()
                .then((a: any) => {
                      dispatch(setTemporaryStorage(a.data))
                    router.push(`/auth/info?code=${values.code}`)
                ...
                <Form>
                    ...
                    >
                        <Center flexDir={"column"}>
                            ...
                            <Box w="full" cursor="point" onClick={() => router.push("/auth/signup")} mt="20px" textDecoration={"underline"} fontFamily={"Poppins"} fontSize={"14px"} textAlign={"end"}>
